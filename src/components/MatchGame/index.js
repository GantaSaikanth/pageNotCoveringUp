...
class MatchGame extends Component {
  ...
  imageClick = url => {
    ...
    const imgValue = imagesList.filter(
      eachValue => eachValue.thumbnailUrl === url,
    )
  ...
  getFilteredLists = () => {
    ...
    const filteredLists = imagesList.filter(
  ...
  render() {
    const {timeElapsedinSeconds, isTrue, score, imgUrl, activeTabId} =
      this.state
    const filteredList = this.getFilteredLists()

    return (
      <div className="bg-container">
        <ul className="navbar">
          <Navbar time={timeElapsedinSeconds} score={score} />
        </ul>
          ...
          <ul className="tab-container">
            {tabsList.map(eachTab => (
              <MatchTabItem
                key={eachTab.tabId}
                tabsList={eachTab}
                onClickTab={this.onClickTabList}
          ...
          <ul className="thumbnail-container">
            {filteredList.map(eachImage => (
              <MatchItems
                ...
                onClickImagesList={this.imageClick}
              />
            ))}
          </ul>
        )}
